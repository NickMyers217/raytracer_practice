/** Example *
 *  
 *  The purpose of this example is just to have fun with what we've learned so far
 *
 *  This will show how a ray tracer can actually create something cool too look at
 *
 *  We will accomplish this by using our spheres to create a space scene.
 *
 */
...
const trace = function(ray, objs, lights) {
    ...
    let pColor = new Vec3(0, 0, 0); // The final color of the pixel
...
const render = function(img, objs, lights) {
    ...
    for(let y = 0; y < height; y++) {
        for(let x = 0; x < width; x++) {
            ...
            // The spheres are planets, the background will be stars
            if(wasHit) {
                // Its a planet so the color we got back is fine
                let b = y * 4 * width + x * 4;
                ...
                // Its part of the background
                // Lets decide if it should be a star or not
                // 1 in 1000 chance
                const starBias = Math.floor(Math.random() * 1000);
                
                if(starBias == 50) {
                    // Its a star
                    // Lets pick from 3 colors
                    const starColBias = Math.floor(Math.random() * 2);
...
window.onload = () => {
    ...
    let lights = [];
    lights.push(new PointLight(new Vec3(-25,0,2), new Vec3(1,1,1), 4));
